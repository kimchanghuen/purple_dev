...
<script type="text/javascript">
	...
	function pageMove(idx, code, name, enable) {
		...
		f.action = "/Kind/KindUpdate";//이동할 페이지
...
<body>
	...
	%>
	<form name="param">
		<input type="hidden" name="k_idx"> <input type="hidden"
			name="k_code"> <input type="hidden" name="k_name"> <input
			type="hidden" name="k_enable">
	</form>


	<div id="top_header">
		<jsp:include page="../Datacontroller/NewtopMenu_sub1.jsp"
			flush="false" />
	</div>

	<div id="buttonDiv" style="text-align: center;">
		<button type="button" onclick="location.href='/Kind/Kindinsert' "
	...
	<div id="dataField"
		...
		<table border='1'
			...
			<tbody>
				<c:forEach items="${list}" var="KindList">
					<tr>
						<td><c:out value="${KindList.k_code}" /></td>
						<td><c:out value="${KindList.k_name}" /></td>
						<td><fmt:formatDate value="${KindList.k_date}"
								pattern="yyyy-MM-dd" /></td>

						<td width="70">
							<button type="button"
								onclick="pageMove('${KindList.k_idx}'
								,'${KindList.k_code}'
								,'${KindList.k_name}'
								,'${KindList.k_enable}'
								) ">수정</button>
