...
<script type="text/javascript">
	...
	// 각 데이터 클릭시
	function Type_change(code, name, enable) {
		var f = document.param; //폼 name

		f.t_code.value = code;
		f.t_name.value = name;
		f.t_enable.value = enable;

		f.action = "/Type/TypeUpdate";//이동할 페이지
...
<body>
	...
	<div id="header">
		<jsp:include page="../Datacontroller/NewtopMenu_sub4.jsp"
			flush="false" />
	</div>
	...
	<form name="param">
		<input type="hidden" name="t_idx"> <input type="hidden"
			name="t_code"> <input type="hidden" name="t_name"> <input
			type="hidden" name="t_enable">
	</form>

	<div id="dataField"
		style="width: 870px; text-align: center; margin: auto;">

		<button type="button" onclick="location.href='/Type/Typeinsert' "
		...
		<table border='1'
			...
			<thead>
				...
				<script>
					...
					document.write('<th width="160" >' + tableHdr[1]
							+ ' <span id="sort_1"></span> </th>'); // name
					document.write('<th width="180" >' + tableHdr[2]
							+ ' <span id="sort_2"></span> </th>'); // 수정일자
					document.write('<th width="70" >' + tableHdr[3]
							+ ' <span id="sort_3"></span> </th>'); // 수정
				</script>
			</thead>
			...
			<tbody>
				<c:forEach items="${list}" var="TypeList">
					<tr>
						...
						<td><c:out value="${TypeList.t_code}" /></td>
						<td><c:out value="${TypeList.t_name}" /></td>
						<td><fmt:formatDate value="${TypeList.t_update}"
								pattern="yyyy-MM-dd" /></td>

						<td width="70">
							<button type="button"
								onclick="Type_change('${TypeList.t_code}'
								,'${TypeList.strTname}'
								,'${TypeList.t_enable}'
								) ">수정</button>
						</td>
