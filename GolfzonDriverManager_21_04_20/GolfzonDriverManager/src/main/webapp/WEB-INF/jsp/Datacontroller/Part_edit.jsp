...
if(request.getParameter("kind_id")!=null) 
{
    ...
    try
    {
        ...
        query.append( "  SELECT p_idx, p_code, p_name, p_date, p_enable FROM DRV_PART " );        
        ...
        if ( rs.getRow() > 0 ) {
        	...
        	%>
        	 ...
        	 <table border = '1' style="text-align:center; border-style:line;" cellpadding=0 cellspacing=0 id="resultTable">
        	   <thead>
        	     <tr>
		        	<td width="80">p_idx</td>
		        	<td width="80">p_code</td>
		        	<td width="150">p_name</td>
		        	<td width="180">p_date</td>
		        	<td width="80">p_enable</td>
				</tr>
			  </thead>
	        ...
	        while( rs.next() ){
	        	%>
		        	<tbody>
		        		<tr>
				        	<td><%=rs.getString("p_idx") %></td>
				        	<td><input type="text" name="k_code" value="<%=rs.getString("p_code") %>" style="width:80px;" ></td>
				        	<td><input type="text" name="k_name" value="<%=rs.getString("p_name") %>" style="width:150px;" ></td>
				        	<td><%=rs.getString("p_date") %></td>
				        	<td><input type="text" name="k_enable" value="<%=rs.getString("p_enable") %>" style="width:80px;" ></td>
			        	</tr>
		        	</tbody>
	        	<%
	        } 
